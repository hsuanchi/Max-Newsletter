...
def send_mail(send_subject: str, send_from: str, send_to: list,
              send_html_path: str, send_type: str, *args, **kwargs):

    with smtplib.SMTP(host="smtp.gmail.com", port="587") as smtp:  # 設定SMTP伺服器
        try:
            smtp.ehlo()  # 驗證 SMTP 伺服器
            smtp.starttls()  # 建立加密傳輸
            smtp.login(os.environ.get('email_username'),
            ...
            for user in send_to:
                ...
                if send_type == 'multiple':
                    ...
                    content["To"] = user
                    html = send_schedule_mail(send_html_path,
                                              user=user,
                                              *args,
                                              **kwargs)
                # Send Validate Email
                    ...
                    content["From"] = send_from
                    content["To"] = user
                    html = render_template(send_html_path,
                                           user=user,
                                           *args,
                                           **kwargs)
...
def send_schedule_mail(send_html_path, user, *args, **kwargs):
    ...
    unsubscribe_email_path = current_app.config[
        'HOST_NAME'] + f'/en/email/unsubscribe?jwt_token={jwt_token}'
